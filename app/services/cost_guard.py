...
def estimate_cost(goal: str, context: Optional[str] = None) -> int:
    ...
    logger.info(
        f"Token estimation: goal={goal_tokens}, context={context_tokens}, "
...
def check_cost_limits(estimated_tokens: int) -> None:
    ...
    if estimated_tokens > MAX_INPUT_TOKENS:
        logger.warning(
            f"Request rejected: {estimated_tokens} tokens exceeds limit of {MAX_INPUT_TOKENS}"
        )
        raise HTTPException(
    ...
    logger.info(
        f"Estimated cost: ${total_cost:.4f} (input: ${input_cost:.4f}, output: ${output_cost:.4f})"
    )

    if total_cost > 0.50:
        logger.warning(
            f"Request rejected: estimated cost ${total_cost:.4f} exceeds per-request limit"
        )
        raise HTTPException(
            status_code=status.HTTP_413_REQUEST_ENTITY_TOO_LARGE,
            detail={
