...
class WeeklyBreakdown(BaseModel):
    ...
    tasks: List[WeeklyTask] = Field(
        ..., min_length=1, description="Tasks for this week"
    )
...
class GeneratePlanResponse(BaseModel):
    ...
    goal: str
    category: str = Field(
        ..., description="Auto-classified category (e.g., skill-development)"
    )
    ...
    metadata: dict = Field(
        default_factory=dict, description="Internal metadata (tokens used, cost, etc.)"
    )
